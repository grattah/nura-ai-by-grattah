...
describe("Base Nutrition Score — PRD worked examples", () => {
  it("Almond Maca Shake (Beverage, Blended) → 5.9 / 10", () => {
    const r = scoreBaseNutrition({
      ...
      track: "Beverage",
      preparation: "Blended",
      energy_kcal: 350 / 4.184,
      total_sugar_g: 12,
      added_sugar_g: 0,
      sat_fat_g: 0.22,
      sodium_mg: 0.094 * 1000 / 2.5,
      protein_g: 1.77,
      fiber_g: 2.31,
      fvl_pct: 34.8,
...
describe("Track / Preparation classification", () => {
  ...
  it("prep-based Juiced/Blended with Juiced default", () => {
    ...
    expect(classifyPreparation("Beverage", "Steep for 5 minutes")).toBe("Juiced");
...
describe("Combining rule — protein exclusion when NegTotal ≥ 11 (Solid)", () => {
  it("excludes protein from the raw score at high negatives", () => {
    const r = scoreBaseNutrition({
      ...
      track: "Solid Food",
      preparation: "N/A",
      energy_kcal: 4000 / 4.184,
      total_sugar_g: 40,
      sat_fat_g: 12,
      sodium_mg: (2.5 / 2.5) * 1000,
      protein_g: 20,
      fiber_g: 10,
      fvl_pct: 0,
    });
