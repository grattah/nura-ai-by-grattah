...
describe("§3 — anniversary day", () => {
  ...
  it("clamps a 31st anchor to the last day of short months", () => {
    const anchor = 31;
    expect(iso(allocationDateInMonth(anchor, 2026, 3))).toBe("2026-04-30");
    expect(iso(allocationDateInMonth(anchor, 2026, 4))).toBe("2026-05-31");
    expect(iso(allocationDateInMonth(anchor, 2027, 1))).toBe("2027-02-28");
  });
...
describe("§7 — lifecycle", () => {
  it("kills subscription units and freezes purchased on lapse", () => {
    ...
    expect(after.purchasedUnits).toBe(25);
    expect(after.purchasedFrozen).toBe(true);
  });
