        ...
        {NAV_ITEMS.map(({ label, href, icon: Icon, activeIcon: ActiveIcon }) => {
          ...
          return (
            <Link
              ...
              href={href}
              className={cn(
                "min-w-0 w-full flex flex-col items-center gap-0.5 py-2 px-3 max-xs:px-1 rounded-full transition-all duration-150 active:scale-95",
                active ? "bg-black/12" : "",
              )}
              aria-label={label}
            >
              ...
              <span
                className={cn(
                  "max-xs:text-[10px] text-sm leading-none transition-colors duration-150 text-nowrap",
                  active
